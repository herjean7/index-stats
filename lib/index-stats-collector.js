class IndexStatsCollector {
   ...
   */
  analyzeIndexEfficiency(indexStats, collectionStats) {
    const analysis = {
    ...
    // Find unused indexes (0 operations)
    analysis.unusedIndexes = indexStats.filter(stat => 
      stat.usage.ops === 0 && stat.name !== '_id_'
    );

    // Find lightly used indexes (< 1 operation per day on average)
    analysis.lightlyUsedIndexes = indexStats.filter(stat => 
      stat.opsPerDay < 1 && stat.usage.ops > 0 && stat.name !== '_id_'
    );
  ...
  _generateIndexRecommendations(analysis) {
    ...
    if (analysis.lightlyUsedIndexes.length > 0) {
      analysis.recommendations.push({
        type: 'review_lightly_used',
        priority: 'medium',
        message: `Review ${analysis.lightlyUsedIndexes.length} lightly used indexes`,
        indexes: analysis.lightlyUsedIndexes.map(idx => idx.name)
      });
    }
