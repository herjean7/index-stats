...
class MongoDBAnalyzer {
  constructor(config) {
    ...
    this.indexStatsCollector = new IndexStatsCollector({ unusedDaysThreshold: this.unusedDaysThreshold });
  ...
  async analyzeNode(endpoint) {
    ...
    try {
      ...
      for (const dbInfo of databasesToAnalyze) {
        try {
          const databaseResult = await this._analyzeDatabase(client, dbInfo.name, nodeResult.lastRestart);
   ...
   */
  async _analyzeDatabase(client, dbName, lastRestart = null) {
    ...
    for (const collectionInfo of collections) {
      if (collectionInfo.type === 'collection' && !collectionInfo.name.startsWith('system.')) {
        try {
          const collectionResult = await this._analyzeCollection(db, collectionInfo.name, lastRestart);
   ...
   */
  async _analyzeCollection(db, collectionName, lastRestart = null) {
   ...
   */
  _generateCollectionRecommendations(indexStats, ttlIndexes, lastRestart = null) {
    const recommendations = [];

    // Use the proper index analysis that considers restart time
    const indexAnalysis = this.indexStatsCollector.analyzeIndexEfficiency(indexStats, null, lastRestart);
    
    if (indexAnalysis.recommendations) {
  ...
  _getConnectionString(endpoint) {
    ...
    if (!this.username || !this.password) {
      throw new Error('MONGODB_USERNAME and MONGODB_PASSWORD are required when endpoint connectionString is not provided');
    }
