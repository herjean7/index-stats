...
class AliCloudClient {
  ...
  /**
   * Get list of MongoDB instances
   * @param {string} instanceId - Optional specific instance ID to filter
   * @returns {Promise<Array>} Array of MongoDB instances
   */
  async getDBInstances(instanceId = null) {
    const endpoint = 'https://mongodb.aliyuncs.com';
    const action = 'DescribeDBInstances';
    ...
    try {
      ...
      if (data && data.DBInstances) {
        const dbInstances = Array.isArray(data.DBInstances.DBInstance) 
          ? data.DBInstances.DBInstance 
          : [data.DBInstances.DBInstance];

        // Filter for MongoDB instances that are running
        return dbInstances.filter(instance => 
          instance.Engine === 'MongoDB' && 
          instance.DBInstanceStatus === 'Running'
        );
      } else {
        return [];
      }
    } catch (error) {
      throw new Error(`Failed to fetch DB instances: ${error.message}`);
    }
  }
  ...
  async getDBInstanceAttribute(instanceId) {
    ...
    try {
      ...
      console.log(`hello: ` + JSON.stringify(connectionEndpoints));
